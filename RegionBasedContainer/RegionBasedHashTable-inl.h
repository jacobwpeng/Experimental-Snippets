...
#ifndef __REGIONBASEDHASHTABLE_H__
...
#define HashTableIteratorType                                                  \
  HashTableIterator<Key, T, Hash, Pred, KeyOfValue>

HashTableIteratorTypeDeclaration
HashTableIteratorType::HashTableIterator()
  :ht_(nullptr), node_(nullptr) {
}
HashTableIteratorTypeDeclaration
HashTableIteratorType::HashTableIterator(_HashTable* ht, _HashTableNode* node)
  :ht_(ht), node_(node) {
}
HashTableIteratorTypeDeclaration
HashTableIteratorType& HashTableIteratorType::operator++() {
  ...
  for (auto index = bucket_index + 1;
      index < ht_->bucket_count();
      ++index) {
...
HashTableIteratorTypeDeclaration
HashTableIteratorType HashTableIteratorType::operator++(int) {
  auto tmp = *this;
  ++*this;
  return tmp;
}
HashTableIteratorTypeDeclaration
typename HashTableIteratorType::_Base::reference
HashTableIteratorType::operator*() const {
  return node_->val;
}

HashTableIteratorTypeDeclaration
typename HashTableIteratorType::_Base::pointer
HashTableIteratorType::operator->() const {
  return &(operator*());
}

HashTableIteratorTypeDeclaration
bool HashTableIteratorType::operator==(const HashTableIterator& rhs) const {
...
HashTableIteratorTypeDeclaration
HashTableIteratorType::HashTableConstIterator()
  :ht_(nullptr), node_(nullptr) {
}
...
HashTableIteratorTypeDeclaration
HashTableIteratorType::HashTableConstIterator(const _HashTable* ht,
    const _HashTableNode* node)
  :ht_(ht), node_(node) {
}
HashTableIteratorTypeDeclaration
HashTableIteratorType& HashTableIteratorType::operator++() {
...
HashTableIteratorTypeDeclaration
bool HashTableIteratorType::operator==(
...
std::unique_ptr<typename HashTableType::_HashTable>
HashTableType::Create(char* data, size_t size) {
  ...
  // (max_size / 2) * sizeof(NodeID) + (max_size) * sizeof(_HashTableNode) = size;
  auto bucket_num = std::max(kPrimeList[0],
...
HashTableTypeDeclaration
bool HashTableType::empty() const {
  return size() == 0;
}

HashTableTypeDeclaration
typename HashTableType::size_type HashTableType::size() const {
  return alloc_->size();
}

HashTableTypeDeclaration
typename HashTableType::size_type HashTableType::max_size() const {
  return alloc_->max_size();
}

HashTableTypeDeclaration
typename HashTableType::size_type HashTableType::bucket_count() const {
  return buckets_->size();
}

HashTableTypeDeclaration
typename HashTableType::size_type 
...
HashTableTypeDeclaration
typename HashTableType::iterator HashTableType::begin() {
...
HashTableTypeDeclaration
typename HashTableType::const_iterator HashTableType::begin() const {
  for (size_t i = 0; i < buckets_->size(); ++i) {
    if ((*buckets_)[i] != AllocatorType::kInvalidNodeID) {
...
HashTableTypeDeclaration
typename HashTableType::const_iterator HashTableType::cbegin() const {
  return begin();
}
...
HashTableTypeDeclaration
typename HashTableType::iterator HashTableType::end() {
  return iterator(this, nullptr);
}

HashTableTypeDeclaration
typename HashTableType::const_iterator HashTableType::end() const {
  return const_iterator(this, nullptr);
}

HashTableTypeDeclaration
typename HashTableType::const_iterator HashTableType::cend() const {
  return end();
}
...
HashTableTypeDeclaration
std::pair<typename HashTableType::iterator, bool>
...
HashTableTypeDeclaration
typename HashTableType::size_type HashTableType::erase(const key_type& k) {
...
HashTableTypeDeclaration
void HashTableType::clear() {
  std::fill(buckets_->begin(), buckets_->end(), AllocatorType::kInvalidNodeID);
  alloc_->clear();
}

HashTableTypeDeclaration
typename HashTableType::iterator HashTableType::find(const key_type& k) {
  auto bucket_index = bucket(k);
  auto id = (*buckets_)[bucket_index];
  while (id != AllocatorType::kInvalidNodeID) {
    auto node = alloc_->Get(id);
...
HashTableTypeDeclaration
HashTableType::RegionBasedHashTable()
  :header_(nullptr) {
}
