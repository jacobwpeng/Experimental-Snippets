...
std::unique_ptr<VectorType> VectorType::Create(char* data,
  ...
  return std::unique_ptr<VectorType>(new VectorType(
        header, capacity, reinterpret_cast<T*>(aligned_data)));
}
...
VectorType::RegionBasedVector(Header* header, size_t capacity, T* start)
  :start_(start), header_(header),
  capacity_(capacity), destroy_func_(nullptr) {
}
...
void VectorType::clear() {
  if (destroy_func_) {
    std::for_each(begin(), end(), [this](T& val){
        destroy_func_(val);
    });
  }
  header_->current = 0;
...
template<typename T>
typename VectorType::DestroyFunc
VectorType::on_destroy(DestroyFunc destroy_func) {
