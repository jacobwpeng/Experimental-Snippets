...
extern "C" {
    ...
    int fx_DynamicArray_Internal_Say(lua_State * L)
    {
        const int __top = lua_gettop(L);
        UserdataWrapper * __ud;
        if (__top == 1)
        {
            bool __check_failed = false;
            fx::DynamicArray::Internal * __class_ptr;
            if (lua_type(L, 1) == LUA_TUSERDATA && (__ud = (UserdataWrapper*)lua_touserdata(L, 1)) && __ud->type_info.hash == 764692061  && __ud->type_info.is_const == false)
            {
                __class_ptr = (fx::DynamicArray::Internal *)(__ud->val);
            }
            else
            {
                __check_failed = true;
    ...
    int luaopen_array(lua_State * L)
    {
        static const struct luaL_Reg libfuncs[] = {
            ...
            {"Say", fx_DynamicArray_Internal_Say},
